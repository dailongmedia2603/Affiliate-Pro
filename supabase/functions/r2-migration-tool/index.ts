...
async function getR2PresignedUrl(fileName, contentType) { 
    ...
    if (!accountId || !accessKeyId || !secretAccessKey || !bucketName) throw new Error("Cloudflare R2 environment variables are not set."); 
...
async function uploadToR2(fileBuffer, fileName, contentType) { 
    ...
    const uploadResponse = await fetch(presignedUrl, { method: 'PUT', headers: { 'Content-Type': contentType }, body: fileBuffer }); 
    if (!uploadResponse.ok) throw new Error(`Failed to upload to R2: ${await uploadResponse.text()}`); 
...
serve(async (req) => { 
    ...
    try { 
        ...
        if (!r2PublicUrl) throw new Error("CLOUDFLARE_R2_PUBLIC_URL is not set."); 
        const summary = {}; 
            ...
            for (const file of files) { 
                ...
                try { 
                    ...
                    if (config.table && config.column) { 
                        ...
                        if (checkError) throw new Error(`DB check failed for ${file.name}: ${checkError.message}`); 
                    ...
                    if (downloadError) throw new Error(`Download failed for ${file.name}: ${downloadError.message}`); 
