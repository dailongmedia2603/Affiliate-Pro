...
serve(async (req) => {
  ...
  try {
    ...
    if (!runId) throw new Error("Thiếu tham số runId.");
    ...
    if (runUpdateError) throw new Error(`Không thể dừng phiên chạy: ${runUpdateError.message}`);

    // Cancel any pending or running steps for this run
    ...
    if (stepsUpdateError) throw new Error(`Không thể hủy các bước đang chạy: ${stepsUpdateError.message}`);
