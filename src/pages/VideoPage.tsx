    ...
    const checkApiKeys = async () => {
      ...
      if (user) {
        ...
        if (error && error.code !== 'PGRST116') {
          showError('Không thể tải cài đặt API.');
        }
  ...
  const renderApiKeyWarning = (serviceName: string) => (
    <div className="w-full h-full p-6 bg-gray-50/50 flex items-center justify-center">
      <Alert variant="destructive" className="max-w-lg">
        ...
        <AlertTitle>Chưa cấu hình API {serviceName}</AlertTitle>
        <AlertDescription>Vui lòng vào trang Cài đặt và thêm thông tin xác thực để sử dụng tính năng này.</AlertDescription>
      </Alert>
    </div>
  );
    ...
    <div className="w-full p-6 bg-gray-50/50">
      ...
      <Tabs value={activeModel} onValueChange={setActiveModel} className="w-full">
            ...
            {!apiKeySet ? renderApiKeyWarning('Higgsfield') : (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                  <VideoGenerationForm model={model.id} onTaskCreated={handleTaskCreated} channelId={channelId} />
                </div>
        ...
        <TabsContent value="veo3" className="mt-6">
          {!veo3ApiKeySet ? renderApiKeyWarning('VEO 3') : (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2">
                <Veo3GenerationForm onTaskCreated={handleTaskCreated} />
              </div>
              <div className="lg:col-span-1">
